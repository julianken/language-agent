...
description: Use when opening or rewriting an implementation issue or plan spec on julianken/language-agent. Triggers on "create issue", "write the issue", "issue spec", "implementation plan issue", or batch prep/planning work. Self-contained for worktree dispatch.
---

# Issue authoring (julianken/language-agent)

**Announce at start:** *"I'm using the issue-authoring skill to draft an implementation-ready issue spec."*

GitHub slug: `julianken/language-agent`. After the issue is posted, **dispatch `issue-plan-review`** — the author must not self-approve in the same pass.
