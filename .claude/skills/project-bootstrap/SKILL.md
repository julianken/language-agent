...
# Project bootstrap (julianken/language-agent)
...
| --- | --- | --- | --- |
| **fill** | Unfilled template — files still contain `{{PLACEHOLDERS}}` | Substitute every placeholder (`language-agent`, `julianken/language-agent`, optional `mFjP4XMgcvdwzloZL9EMze`/`julianken-bot`, …) from `.seed/placeholders.json` to stand up a new product from the scaffold. | **Implemented** — driver is [`scripts/fill-template.sh`](../../../scripts/fill-template.sh). |
