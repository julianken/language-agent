...
description: |
  ...
  in julianken/language-agent. Use when reviewing a pipeline skill PR, reviewing the
...
This skill is **repo-local-only** — it has **no user-level namesake**. Asset review is instance-coupled to `julianken/language-agent`'s skill/agent conventions and the specific pipeline asset classes (P2/P3/P5/P6/P8/P9); a portable twin would carry violin-tools-specific knowledge it should not. There is therefore no user-level ↔ repo-local mirror pair and no user-level no-drift obligation for this skill.

This skill is mirrored to **`julianken/agentic-seed`** with `{{PLACEHOLDERS}}` substituting instance literals (repo slug `julianken/language-agent`, bot identity `@julianken-bot`, label scheme `pipeline`/`skill`/`review-tooling`) so the generic shape is reusable in other repos. The actual `{{PLACEHOLDER}}`-templated copy in `julianken/agentic-seed` is created by the epic's **P7 bulk-mirror PR** (which covers this skill, the P5 engine, the P6 driving skill, and the P2 dashboard in one cross-repo PR). **P4 does not open the cross-repo mirror PR — P7 owns that.**
