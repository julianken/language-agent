...
The generic method here mirrors the user-level `creating-prs` skill (shared across the owner's repos). This repo-local copy is the canonical one for `julianken/language-agent`; the user-level skill is the portable overlay. Per `AGENTS.md` → **Skill ownership** "No-drift rule": a change to either copy must update the other in the **same PR**, and the PR Summary must say so. On conflict, the repo-local copy wins for anything instance-specific; the user-level skill wins for the portable method itself.
