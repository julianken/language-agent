# `.claude/agents/` — Subagent index for `julianken/language-agent`

This directory holds repo-specific subagents for `julianken/language-agent` (local folder
`language-agent/`). Agents placed here are dispatchable via the `Task` tool from a
