...
- **Tone-3 sandhi with explicit word-grouping.** For 3+ stacked third tones the surface form depends on syntactic grouping (你想買 groups as `[你][想買]` vs `[你想][買]`, yielding different sandhi). The sandhi resolver emits an **explicit word-grouping** alongside the surface string, and the gate verifies the surface form *against that grouping* — a surface tone with no grouping for a 3+ third-tone run is a gate failure, not a pass. **But verifying tones *against the resolver's own grouping* makes the grouping itself a single point of trust** — a wrong grouping is self-consistent and would pass GREEN, and no deterministic leg (citation-membership, g2pW base reading, segmenter boundaries) validates grouping. So an ambiguous 3+-third-tone run is **flagged to the hot-path jury (§5.3, §6.4) for an *independent* syntactic-grouping judgment** — the one case the jury runs hot specifically to break the resolver's self-consistency. Jury disagreement on the grouping **downgrades the run to YELLOW** (fade suppressed, margin note), so a mis-grouped surface tone is never shown GREEN/as-mastered; the residual (the jury is independent but not deterministic) is the tracked open verification item in §10.5.
...
**License-verification note (anti-invention).** The contract did not specify licenses for **g2pM, g2pC, jieba, or pkuseg**; the values above are the *expected* upstream licenses **to be confirmed at the license-hygiene build stage (§10.3) before each dependency is admitted** — they are marked *verify upstream* rather than asserted as fact. The g2pM/g2pC ordering is corrected from an earlier draft (g2pM = MIT, g2pC = Apache-2.0, both pending confirmation; an earlier draft had them swapped). The remaining licenses — pypinyin (MIT), g2pW (Apache-2.0), HanLP core (Apache-2.0), OpenCC (Apache-2.0), CKIP-Transformers (GPL-3.0), CC-CEDICT (CC BY-SA), Kaikki/Wiktextract (CC BY-SA), CHISE-IDS/cjkvi-ids (GPL-2.0), Make-Me-a-Hanzi (code MIT; **stroke-order SVG graphics under the Arphic Public License**; derived dict CC BY-SA), FSRS (MIT), pyBKT (MIT) — are as the contract specified. (The Make-Me-a-Hanzi split is corrected to match `docs/research/chinese-validation.md`: the asset we actually consume is the **Arphic-licensed SVG**, so it is *not* a CC BY-SA asset and is routed to the Arphic quarantine, §10.3.)
...
- **Make-Me-a-Hanzi** (**stroke-order SVG graphics under the Arphic Public License**; code MIT; derived dict CC BY-SA; leans Simplified/PRC stroke order) → stroke-order SVG. The asset consumed here is the **Arphic-licensed SVG**, so it is routed to the **Arphic quarantine (§10.3)**, *not* the CC BY-SA bucket. Used for the breakdown panel only; **tagged DORMANT** for handwriting (Traditional needs Taiwan stroke order — see §9.1 production deferral).
...
- **CC BY-SA quarantine + notice:** CC-CEDICT, Kaikki/Wiktextract, and the *derived dictionary* of Make-Me-a-Hanzi (its Unihan + CC-CEDICT-derived data). **Make-Me-a-Hanzi's stroke-order SVG *graphics* are NOT CC BY-SA** — they are Arphic-licensed and belong in the Arphic bucket below.
- **Arphic / font licenses:** Noto Sans/Serif TC ship under OFL (clean); the **Make-Me-a-Hanzi stroke-order SVG graphics (Arphic Public License)** — the asset the breakdown panel actually consumes — and any other Arphic-derived asset are quarantined here with the Arphic notice, governed by the Arphic terms, not CC BY-SA.
...
4. **Tone-sandhi / 3-stacked-third-tone has no single deterministic surface form in isolation.** *Mitigation:* the two-layer pinyin model (citation + surface) with two-stage resolution — g2pW disambiguates the base reading, a rule-based sandhi resolver applies the occurrence-level transforms — and the gate verifying the **surface form in-sentence using explicit word-grouping** for 3+ stacked third tones; first-class neutral-tone and erhua slots; 不/一 shifts resolved at the surface layer. Dictionary membership checks citation only. **Residual single-point-of-trust:** because the gate verifies tones *against* the resolver's chosen grouping, grouping correctness has no deterministic verifier; ambiguous 3+-third-tone runs are therefore **flagged to the hot-path jury for an independent grouping judgment**, with disagreement downgrading to YELLOW (§3.4, §5.3). The remaining exposure is the §10.5 open verification item.
...
- **3rd-tone phrase-grouping verification.** *Open verification item (the one independent-but-not-deterministic leg):* for 3+-stacked-third-tone runs the surface tone depends on a phrase-grouping that **no deterministic gate leg validates** — the gate checks tones against the resolver's *own* grouping (§3.4), so a wrong grouping is self-consistent. *v1 mitigation:* flag every ambiguous such run to the **hot-path jury for an independent grouping judgment**, downgrading to YELLOW (fade suppressed) on disagreement (§3.4, §5.3, §10.4 risk 4) — so a mis-grouped tone is never shown as mastered, only ever scaffolded. Native-checker calibration (Cohen κ) over-weights these runs. A **deterministic syntactic-grouping check** (e.g. a parser-derived prosodic-word bracketing the gate could compare against) is a tracked follow-up, not a v1 blocker.
